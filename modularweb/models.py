...
class MainPage(BasePage):
    contentPages = models.ManyToManyField(
        ContentPage,
        blank=True,
        related_name='sub_pages')

    def get_content_pages(self):
        main_content_pages = MainContentPage.objects.filter(
            mainPage=self).all()
...
class Gallery(models.Model):
    ...
    def GetGalleryPhoto(self, photoSlug):
        photoGallery = GalleryPhotographies.objects.filter(
            gallery=self,
            photography__slug=photoSlug).first()
...
class GalleryPage(BasePage):
    ...
    def getGalleryPhotographies(self):
        galleryPhotos = GalleryPhotographies.objects.filter(
