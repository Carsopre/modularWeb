...
class TestPhotography(TestCase):
    ...
    def test_photography_returns_title(self):
        ...
        self.assertEqual(
            self.__test_photography.__str__(),
            self.__test_photography.title)

    @parameterized.expand([
        ('case_none_value', None),
        ('case_empty_string', ''),
        ('case_wrong_slug', '42')])
    def test_get_photography_without_arguments_does_not_raise_and_returns_none(
    ...
    def test_get_photography_with_valid_slug_returns_expected_photography(
            self):
        ...
        # 3. Verify expectations
        self.assertEqual(
            found_photography, expected_photography,
            'Expected {} but got {}'.format(
                expected_photography, found_photography))

    @parameterized.expand([
        ('case_none_value', None),
        ('case_empty_string', ''),
        ('case_wrong_slug', '42')])
    def test_get_photography_url_no_arguments_doesnot_raise_and_returns_empty(
            self, case_name, search_slug):
        # 1. Set up variables
        found_url = None
        expected_ulr = ''
        ...
        # 3. Verify expectations
        self.assertEqual(
            found_url, expected_ulr,
            'Expected {} but got {}'.format(expected_ulr, found_url))
    ...
    def test_get_photography_url_with_valid_slug_returns_expected_url(self):
        ...
        try:
            ...
            self.fail(
                'Not expected exception, but thrown: {}'.format(str(e_error)))

        # 3. Verify expectations
        self.assertEqual(
            found_url, expected_url,
            'Expected {} but got {}'.format(expected_url, found_url))
